    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                ...
                $total_pages_sql = "SELECT COUNT(*) FROM enseignant_soutenance";
                $result = mysqli_query($link, $total_pages_sql);
                $total_rows = mysqli_fetch_array($result)[0];
                $total_pages = ceil($total_rows / $no_of_records_per_page);

                //Column sorting on column name
                $orderBy = array('id_enseignant', 'date', 'temps');
                $order = 'id';
                ...
                $sql = "SELECT * FROM enseignant_soutenance ORDER BY $order $sort LIMIT $offset, $no_of_records_per_page";
                $count_pages = "SELECT * FROM enseignant_soutenance";


                if (!empty($_GET['search'])) {
                    $search = ($_GET['search']);
                    $sql = "SELECT * FROM enseignant_soutenance
                            WHERE CONCAT (id_enseignant,date,temps)
                            LIKE '%$search%'
                            ORDER BY $order $sort
                ...
                if ($result = mysqli_query($link, $sql)) {
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        while ($row = mysqli_fetch_array($result)) {
                            ...
                            echo "<td>";
                            echo "<a href='enseignant_soutenance-read.php?id=" . $row['id'] . "' title='View Record' data-toggle='tooltip'><i class='far fa-eye'></i></a>";
                            echo "<a href='enseignant_soutenance-update.php?id=" . $row['id'] . "' title='Update Record' data-toggle='tooltip'><i class='far fa-edit'></i></a>";
                            echo "<a href='enseignant_soutenance-delete.php?id=" . $row['id'] . "' title='Delete Record' data-toggle='tooltip'><i class='far fa-trash-alt'></i></a>";
                            echo "</td>";
                            echo "</tr>";
                        }
                        echo "</tbody>";
